...
def sub_cb(topic, msg):
    ...
    '''
    if topic == MQTT_CONFIG["SUB_COMMAND"] :
        msgStr = msg.decode('utf-8')
        spike_write(msgStr)
    elif topic == MQTT_CONFIG["SUB_INFO"] : 
        pass
...
def connect_and_subscribe():
    ...
    '''
    global client
    client = MQTTClient(client_id = MQTT_CONFIG["CLIENT_ID"], server = MQTT_CONFIG["BROKER"], port = MQTT_CONFIG["PORT"])
    client.set_callback(sub_cb)
    client.connect()
    print('Connected to %s MQTT broker' % (MQTT_CONFIG["BROKER"]), end=" ")
    subscribe(client)
    return client
...
def main():
    ...
    try:
        ...
        client.publish(MQTT_CONFIG["PUB_STATUS"], "MQTT Setup")
    except OSError as e:
    ...
    while True:
        ...
        if (spikeMsg == b'\x03\r\n' or spikeMsg == b'\x03'):
           ...
           break
        if spikeMsg != b'':
            client.publish(MQTT_CONFIG["PUB_POSITION"], spikeMsg)
